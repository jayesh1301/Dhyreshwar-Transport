...
const Branches = () => {
  const columns = [
    { field: "branch_id", headerName: "Id" },
    { field: "srNo", headerName: "SR.No" },
    { field: "branch_code", headerName: "Code", flex: 1 },
    { field: "branch_abbreviation", headerName: "Abbreviation", flex: 1 },
    ...
    { field: "place", headerName: "Place", flex: 1 },
    {
      ...
      renderCell: (params) => {
        const onClick = (e) => {
          e.stopPropagation();
          return navigateToEdit(params.row.branch_id);
        };

        const triggerDelete = (e) => {
          e.stopPropagation();
          return deleteBranch(params.row.branch_id);
        };
  ...
  const user = useSelector((state) => state.user.permissions.Admin.Branch);
  ...
  const fetchData = () => {
    ...
    dispatch(getBranchesByPage({ temp }))
      .then(({ payload = {} }) => {
  ...
  const searchData = () => {
    ...
    dispatch(getBranchesBySearch({ temp }))
      .then(({ payload = {} }) => {
        const { message } = payload?.data || {};
        if (message) {
          setHttpError(message);
    ...
    if (e.target.value === "true") {
      dispatch(removeBranch(selectedId))
        .then(({ payload }) => {
          ...
          setSnackColour(message.includes("Successfully") ? 'success' : 'error')
          fetchData();
        })
        .catch(() => {
          setHttpError(
