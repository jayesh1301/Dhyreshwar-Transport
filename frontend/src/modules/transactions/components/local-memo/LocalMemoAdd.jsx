...
const LocalMemoAdd = () => {
  ...
  const [loadingSlip, setLoadingSlip] = useState({
    ...initialState,
    branch: window.localStorage.getItem('branch') && isSuperAdminOrAdmin() ? JSON.parse(window.localStorage.getItem('branch')) : state,
  });
  ...
  useEffect(() => {
    ...
    const fetchLorryReceipts = async () => {
      if (isModule) {
        setLoading(true);
        try {
          const { payload = {} } = await dispatch(getLorryReceiptsForLocalMemo({
            page,
            isLocalMemo,
  ...
  const submitHandler = (e, isSaveAndPrint) => {
    ...
    if (!validateForm(loadingSlip)) {
      ...
      // console.log("local memo after update: ", updatedLoadingSlip)
      setLoading(true)
      dispatch(createLocalMemo(updatedLoadingSlip))
        .then(({ payload = {} }) => {
          const message = payload?.data?.[0]?.[0]?.message || '';
          const inserted_id = payload?.data?.[1]?.[0]?.inserted_id || null;

          console.log(message, inserted_id)
          // if (message) {
          //   setHttpError(message);
